...
CV tailoring and cover letter writing (per `cv-templates.md` and
`cover-letter-templates.md`) produce two artifacts. Many applications need a **third**
— free-text fields typed directly into an application portal. Large-employer ATS
systems and startup forms routinely ask for things neither document covers, under a
character or word limit, in a box with no formatting.

This file governs that third artifact. It is not a document you compile; it is text the
candidate pastes.
...
**Every claim in a form field must already be defensible from the same sources the CV and cover letter are grounded against** — the union of `Application_Answers.md` and `Application Materials/CV.md`, with a claim grounded if either supports it. The interviewer reads the form alongside the CV. A form field is not a place to introduce new claims, inflate scope, or fill space — it is a place to *select* from what is already true and arrange it for the question asked.

All accuracy rules from `cv-templates.md` and `writing-style.md` apply unchanged.
...
**Dates.** The dates they worked on **that project**, which are not automatically the employment dates. If a role spanned two years but the named project occupied the later part, saying so is both more accurate and avoids a low-output reading (see `cv-templates.md`'s "tenure-vs-output check"). Only narrow the dates when the candidate can say when the project actually started — never invent a boundary to improve the ratio.
...
**Scope discipline is stricter here than on a CV.** A CV bullet can be terse enough to be ambiguous about ownership. A project entry with the candidate's name and role attached reads as ownership of the whole thing. Where they contributed rather than owned (e.g. a group project with a wider team), say so inside the description.
...
- [ ] Every factual claim traces to `Application_Answers.md` or `Application Materials/CV.md`
